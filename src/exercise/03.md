...
React when having a Suspense, suspends a loading fallback for suspended component for a tiny bit (not bug) just as a feature in case the suspended component unsuspends fast enough so there's no need to bother a user with a loading fallback.
...
In the exercise, there’s a usage of an older version of useTransition, so keep that in mind.
While creating a transition, what might happen is that we might get a flash, it can be handled by css (applying transition delay on opacity change for example) (but it might be already handled by Reach built in transition features)

The feature with adding properties to SUSPENS_CONFIG is outdated
