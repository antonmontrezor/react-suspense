...
A robust abstraction for caching -> https://github.com/TanStack/query 

Might be a repetition, but defaultValue of React.createContext(defaultValue) is only used when there’s no matching Provider for a component that uses the context with useContext.
...
Essentially, it's a good idea to store cache within a component so that we can manage and access it during the React component lifecycle
This exercise gives a good understanding about cache invalidation and how to store cache having the access to React lifecycle
