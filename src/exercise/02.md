...
It's convenient to have an API that handles a resource fetch like createResource using it together with Suspense and ReactBoundary instead of having the statuses of a fetch
handled across components.

Fetch on render vs Fetch then render
Both approaches are similar to some point. In first, we render components and then fetch data while showing different UIs based on the fetch status. The resource is managed within the component that shows different UIs based on the above-mentioned conditions. The second approach, we manage the resource in parent component through a fetch API and show different UIs on the same component level as with the child component that should show a finished resource. Also, in the parent different UIs are shown based on the status of the resource managed in the fetch API.
